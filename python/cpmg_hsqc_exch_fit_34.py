...
'''
Modify some parameters listed from line 27 to line 69.

Type as follows for the execution.
The functions for starting the program are listed at the end of this program.

reset -f
run cpmg_hsqc_exch_fit_34.py

Alternatively, from a Linux shell, not from Jupyterlab
date; python3 cpmg_hsqc_exch_fit_34.py >& log.txt; date
'''
...
class Ham:
	...
	dwsb = 80		# S's chemical shift difference of state B from that of state A (Hz)
...
def rex_vs_vcpmg(hamr=None, grp=1, flw=1, noise=0):
...
def two_cpmg(n1, n2, n3, n4, p1, p2, p3, tcp, ldlt, lpa, lkex, lrx, ldw, jdlt, gdlt):
	...
	a = pint1[0]/pint0[0]
	if a <= 0:
		print("Warning: logarithm becomes infinity in two-cpmg during fitting, Rex becomes negative.")
		return 1000
	else:
...
def cpmg_plot(n, fnam, maxnc, pa, kex, rx, dw, tcp, sn, p1, p2, p3, jdlt, gdlt, drex, drey, grp=1, flw=0):
	...
	if grp == 1:
		...
		ax.plot(xnew, ynew(xnew), color="magenta", label="fit", linestyle='dotted', linewidth = 2.0)
		ax.set_xlabel("CPMG frequency (Hz)", fontsize=12)
		ax.set_ylabel("Rex (/s)", fontsize=12)
		# ax.set_ylim([-5, 145.0])
		# plt.show()
		plt.savefig("testtest.pdf", dpi=1000, bbox_inches="tight", transparent=True) ##### must delete plt.show()
...
def calc_resd_pa_fixed(prm, npx, npy, **kwargs):
	...
	for i in range(len(npx)):
		...
		ldlt = 1.0/4.0/npx[i]
		lnc = int(2.0*tcp*npx[i]/sn + 0.5)
		resd[i] = npy[i] - two_cpmg(n1=lnc, n2=lnc, n3=lnc, n4=lnc, p1=p1, p2=p2, p3=p3, tcp=tcp, ldlt=ldlt, lpa=lpa, lkex=prm[0], lrx=prm[1], ldw=prm[2], jdlt=jdlt, gdlt=gdlt)
	return resd
...
def calc_cost(prm, npx, npy, pnum, **kwargs):
	...
	if pnum == 4:
		...
		resd = calc_resd_pa_fixed(prm, npx, npy, **kwargs)
	else:
...
def fit_cpmg(tcp, sn, p1, p2, p3, jdlt, gdlt, drex, drey, hamr=None):
	...
	for pm in range(5):
		...
		try:
			lpa = max(minpa+0.01, min(hamr.kba/(hamr.kab + hamr.kba) + 0.3*np.random.randn(), maxpa-0.01))
		except ZeroDivisionError:
			lpa = 0.999

		p0 = [lpa, lkex, lrx, ldw]

		print("Initial parameters pa kex R0 dw: ", p0)
		...
		print(f"Fit  pa {res.x[0]:.3f}    kex {res.x[1]:.2f} Hz   rx0 {res.x[2]:.2f} Hz    dw {res.x[3]:.2f} Hz    hpi {hamr.hpi:d}")
	...
	print(f"*** Final fit   rmsd {minrmsd:.3f}    pa {pa:.2f}    kex {kex:.2f} Hz   rx0 {rx:.2f} Hz    dw {dw:.2f} Hz\n")
	return pa, kex, rx, dw, minrmsd

def fit_cpmg_pa_fixed(tcp, sn, p1, p2, p3, jdlt, gdlt, drex, drey, pa, hamr=None):
	...
	bd = ([minkex, minrx, mindw], [maxkex, maxrx, maxdw])	# -np.inf
	x = np.array(drex)
	y = np.array(drey)
	kwargs={'tcp':tcp, 'sn':sn, 'p1':p1, 'p2':p2, 'p3':p3, 'jdlt':jdlt, 'gdlt':gdlt, 'pa':pa}
	...
	for pm in range(5):
		...
		# print("Initial parameters pa (fixed) kex R0 dw: ", pa, p0)

		res = least_squares(calc_resd_pa_fixed, p0, loss='soft_l1', bounds=bd, args=(x, y), kwargs=kwargs)
		rmsd = calc_cost([res.x[0], res.x[1], res.x[2]], x, y, 3, **kwargs)
		# print(f"Fit  pa-fixed {pa:.3f}    kex {res.x[0]:.2f} Hz   rx0 {res.x[1]:.2f} Hz    dw {res.x[2]:.2f} Hz    hpi {hamr.hpi:d}")
		# print(f" Cost {res.cost:.10f}")
		# print(f" rmsd {rmsd:.10f}")
		if rmsd < minrmsd or pm == 0:
			kex, rx, dw = res.x[0], res.x[1], res.x[2]
			minrmsd = rmsd

	print(f"*** Final fit   rmsd {minrmsd:.3f}    pa (fixed) {pa:.2f}    kex {kex:.2f} Hz   rx0 {rx:.2f} Hz    dw {dw:.2f} Hz\n")
	return kex, rx, dw, minrmsd
...
def fit_cpmg_2B0(tcp1, tcp2, num1, b0ratio, sn, p1, p2, p3, jdlt, gdlt, drex, drey):
	...
	print(f"*** Final fit  rmsd {minrmsd:.3f}    pa {pa:.2f}    kex {kex:.2f} Hz   rx01 {rx1:.2f} Hz  rx02 {rx2:.2f} Hz   dw {dw:.2f} Hz\n")
...
def start_cpmg_sim_fit_pa_scan(grp=0, noise=0):
	...
	ini_kab = Ham.kab
	ini_kba = Ham.kba
	ini_kex = ini_kab + ini_kba

	drep = []
	drer = []

	with open("cpmg_vs_offs_p.txt", "w") as fpp:
		print("# pm  pa  kex  rx  dw  rmsd", file=fpp)

		for pap in range(1, 100, 2):	# pap should not take 0 or 100.

			pa = pap/100.0
			drep.append(float(pa))
			kba = pa * ini_kex /100.0
			Ham.kab = ini_kex - kba
			Ham.kba = kba

			# The max number of the spin-echo (ldlt - Pi - ldlt)lnc, L6
			kex, rx, dw, rmsd = fit_cpmg_pa_fixed(Ham.tex, Ham.sn, p1, p2, p3, jdlt, gdlt, drex, drey, pa)
			drer.append(float(rmsd))

			if grp == 1:
				cpmg_plot(2, 'mag_out.txt', maxnc, pa, kex, rx, dw, Ham.tex, Ham.sn, p1, p2, p3, jdlt, gdlt, drex, drey, grp=1, flw=0)

			print("{:.2f}  {:.3f}  {:.3f}  {:.3f}  {:.3f}  {:.3f}".format(pa, pa, kex, rx, dw, rmsd), file=fpp)

	# fig = plt.figure(dpi=120)
	# ax = fig.add_subplot(1, 1, 1)
	# ax.scatter(drep, drer, s=4, color="blue", label="measured")
	# ax.axvline(x = ini_kba/ini_kex, ls='--', lw=1)
	# ax.set_ylim([-0.1, 1.1])
	# ax.set_xlabel("p$_{a}$", fontsize=12, fontstyle='italic')

	# plt.show()
	# plt.tight_layout()
	# plt.savefig("test_pa_scan.pdf", dpi=1000, bbox_inches="tight", transparent=True)		# must delete plt.show()

	Ham.kab = ini_kab
	Ham.kba = ini_kba
	start_cpmg_sim_param_plot_2d(pam='pa')
...
def start_cpmg_sim_param_cal_pa():
	...
	with open("cpmg_vs_offs_v.txt", "w") as fpv, open("cpmg_vs_offs_p.txt", "w") as fpp:
		...
		for pap in range(1, 100, 2):	# pap should not take 0 or 100.
			kba = pap * (ini_kab + ini_kba) /100.0
			Ham.kab = ini_kab + ini_kba - kba
			Ham.kba = kba

			pa, kex, rx, dw, rmsd, drex, drey = start_cpmg_sim_fit(grp=0, noise=0)
			print("pm = {:.2f}".format(pap/100.0), file=fpv)
			for i in range(len(drex)):
				print("{:.2f}  {:.3f}  {:.3f}".format(pap/100.0, drex[i], drey[i]), file=fpv)
			print("mp", file=fpv)
			print("{:.2f}  {:.3f}  {:.3f}  {:.3f}  {:.3f}  {:.3f}".format(pap/100.0, pa, kex, rx, dw, rmsd), file=fpp)

	Ham.kab = ini_kab
	Ham.kba = ini_kba

def start_cpmg_sim_param_cal_kex():
	...
	with open("cpmg_vs_offs_v.txt", "w") as fpv, open("cpmg_vs_offs_p.txt", "w") as fpp:
		...
		for pkex in range(1, 4000, 100):	# pkex should not take 0.
			Ham.kab = pkex * ini_kab / (ini_kab + ini_kba)
			Ham.kba = pkex - Ham.kab
			pa, kex, rx, dw, rmsd, drex, drey = start_cpmg_sim_fit(grp=0, noise=0)
			print("pm = {:.2f}".format(pkex), file=fpv)
			for i in range(len(drex)):
				print("{:.2f}  {:.3f}  {:.3f}".format(pkex, drex[i], drey[i]), file=fpv)
			print("mp", file=fpv)
			print("{:.2f}  {:.3f}  {:.3f}  {:.3f}  {:.3f}  {:.3f}".format(pkex, pa, kex, rx, dw, rmsd), file=fpp)

	Ham.kab = ini_kab
	Ham.kba = ini_kba

def start_cpmg_sim_param_cal_dw():
	...
	with open("cpmg_vs_offs_v.txt", "w") as fpv, open("cpmg_vs_offs_p.txt", "w") as fpp:
		...
		for pdw in range(1, 250, 5):
			Ham.dwsb = pdw
			pa, kex, rx, dw, rmsd, drex, drey = start_cpmg_sim_fit(grp=0, noise=0)
			print("pm = {:.2f}".format(pdw), file=fpv)
			for i in range(len(drex)):
				print("{:.2f}  {:.3f}  {:.3f}".format(pdw, drex[i], drey[i]), file=fpv)
			print("mp", file=fpv)
			print("{:.2f}  {:.3f}  {:.3f}  {:.3f}  {:.3f}  {:.3f}".format(pdw, pa, kex, rx, dw, rmsd), file=fpp)
...
def start_cpmg_sim_param_plot_2d(pam='ofs', b0=1):
	...
	for dat in datalist:
		...
		if (len(sdat) == 0):	# only space, only tab, etc.
			...
			if b0 == 1:
				...
				drer.append(float(sdat[5]))
				dred.append(float(sdat[6]))
	...
	if pam == 'pa':
		ax1.plot(drex, drex, ls='--', lw=1)
	else:
		ax1.axhline(y = Ham.kba/(Ham.kab + Ham.kba), ls='--', lw=1)
	...
	if pam == 'kex':
		ax2.plot(drex, drex, ls='--', lw=1)
		ax2.set_ylim([0, 4050])
	else:
		ax2.axhline(y = Ham.kab + Ham.kba, ls='--', lw=1)
		ax2.set_ylim([0, 1520])
	...
	if pam == 'R0':
		ax3.plot(drex, drex, ls='--', lw=1)
		ax3.set_ylim([-0.1, 100])
	else:
		ax3.axhline(y = Ham.r2s, ls='--', lw=1)	# the values are for 500 MHz.
	...
	if pam == 'dw':
		ax4.plot(drex, drex, ls='--', lw=1)
		ax4.set_ylim([0, 455])
	else:
		ax4.axhline(y = Ham.dwsb, ls='--', lw=1)
		ax4.set_ylim([0, 200])

	if pam == 'ofs':
		ax4.set_xlabel("offset (Hz)", fontsize=12, fontstyle='italic')
	elif pam == 'pa':
		ax4.set_xlabel("p$_{a}$", fontsize=12, fontstyle='italic')
	elif pam == 'kex':
		ax4.set_xlabel("k$_{ex}$ (/s)", fontsize=12, fontstyle='italic')
	elif pam == 'dw':
		ax4.set_xlabel("Δω (Hz)", fontsize=12, fontstyle='italic')
	...
	if pam == 'ofs':
		...
		ax5.set_xlabel("p$_{a}$", fontsize=12, fontstyle='italic')
	elif pam == 'kex':
		ax5.set_xlabel("k$_{ex}$ (/s)", fontsize=12, fontstyle='italic')
	elif pam == 'dw':
		ax5.set_xlabel("Δω (Hz)", fontsize=12, fontstyle='italic')

	ax5.set_ylabel("rmsd (/s)", fontsize=12, fontstyle='italic')
	ax5.set_ylim([-0.3, 8.1])
...
def start_cpmg_sim_param_plot_3d(pam='ofs'):
	...
	# fontsize=10
	ax.set_xlabel("$ν_{cpmg}$ (Hz)", fontstyle='italic')
	ax.set_zlabel("R$_{2}^{eff}$ (/s)", fontstyle='italic')

	if pam == 'ofs':
		ax.set_ylabel("offset (kHz)", fontstyle='italic')
	elif pam == 'pa':
		ax.set_ylabel("p$_{a}$", fontstyle='italic')
	elif pam == 'kex':
		ax.set_ylabel("k$_{ex}$ (/s)", fontstyle='italic')
	elif pam == 'dw':
		ax.set_ylabel("dw (Hz)", fontstyle='italic')
...
def start_cpmg_sim_param_plot_3d_2B0(pam='ofs'):
	...
	for dat in datalist:
		...
		if sdat[0] == 'pm':
			if pam == 'ofs':
				drep1.append(float(sdat[2])/1000)
			else:
	...
	for dat in datalist:
		...
		if sdat[0] == 'pm':
			if pam == 'ofs':
				drep2.append(float(sdat[2])/1000)
			else:
				drep2.append(float(sdat[2]))
			drey2 = []
			npm2 += 1
	...
	ax1.set_xlabel("$ν_{cpmg}$ (Hz)", fontsize=10, fontstyle='italic')
	# ax1.set_zlabel("R$_{2}^{eff}$ (/s)", fontsize=10, fontstyle='italic')
	# ax1.set_ylabel("offset (kHz)", fontsize=10, fontstyle='italic')
	# ax1.set_title('R$_{2}^{eff}$ as a function of ν$_{cpmg}$ and offset', fontstyle='italic')
	# ax1.set_title('AFTAC', fontstyle='italic')				# hpi: 8
	# ax1.set_title('Conventional RC', fontstyle='italic')	# hpi: 7
	# ax1.set_title('CW-CPMG', fontstyle='italic')			# hpi: 6
	ax1.set_title('ST-CW-CPMG  R$_{2}^{eff}$ (/s)', fontstyle='italic')		# hpi: 5
	ax1.set_zlim([-3, 35])
	# ax1.set_zlim([-1, 12])

	ax2.set_xlabel("$ν_{cpmg}$ (Hz)", fontsize=10, fontstyle='italic')
	# ax2.set_zlabel("R$_{2}^{eff}$ (/s)", fontsize=10, fontstyle='italic')

	if pam == 'ofs':
		ax2.set_ylabel("offset (kHz)", fontsize=10, fontstyle='italic')
	elif pam == 'pa':
		ax2.set_ylabel("p$_{a}$", fontsize=10, fontstyle='italic')
...
def start_cpmg_plot_2d(finnam='mag.dat', foutnam="test.pdf"):
	...
	xnew = np.linspace(min(drex), max(drex), num=100)
	ax.plot(xnew, ynew(xnew), color="magenta", label="fit", linestyle='dotted', linewidth = 2.0)
	ax.set_xlabel("$ν_{cpmg}$ (Hz)", fontsize=12, fontstyle='italic')
...
def start_cpmg_sim_param_cal_fullauto_offset():
	...
	for hpi in [3, 4, 5, 6, 7, 8]:
		...
		start_cpmg_sim_param_plot_2d(b0=2)
...
def start_cpmg_sim_param_cal_fullauto_pam(pam):
	...
	for hpi in [3, 4, 5, 6, 7, 8]:
		...
		dir = "hpi_" + str(hpi) + "_" + pam			# with exchange, 1 B0
		os.makedirs(dir, exist_ok=True)
		os.chdir(dir)

		if pam == 'pa':
			start_cpmg_sim_param_cal_pa()
		elif pam == 'kex':
			start_cpmg_sim_param_cal_kex()
		elif pam == 'dw':
			start_cpmg_sim_param_cal_dw()
		else:
			pass

		start_cpmg_sim_param_plot_2d(pam=pam)
		start_cpmg_sim_param_plot_3d(pam=pam)
		shutil.move('test2d.pdf', dir + '_test2d.pdf')
		shutil.move('test3d.pdf', dir + '_test3d.pdf')
		plt.clf()
		plt.close("all")
...
#start_cpmg_sim_param_plot_2d(b0=2)						# 2D plot of pa, Kex, R0, dw
#start_cpmg_sim_param_plot_3d_2B0()						# 3D plot of RD curves against offset on two B0 static magnetic fields

#start_cpmg_sim_param_cal_fullauto_offset()				# Automatically creating a series of figures (dependency on offset) for the paper
#start_cpmg_sim_param_cal_fullauto_pam('pa' or 'kex' or 'dw')	# Automatically creating a series of figures (dependency on pa) for the paper
#start_cpmg_sim_param_cal_fullauto_pam('pa')
#start_cpmg_sim_fit_pa_scan()							# the same as start_cpmg_sim_fit(), but pa is fixed and scanned during optimization
