...
reset -f
run cpmg_hsqc_exch_fit_28.py

Alternatively, from a shell, not from Jupyterlab
python3 cpmg_hsqc_exch_fit_28.py >& log.txt
'''
...
class Ham:
	...
	tex = 30*10**-3	# Total relaxation delay = 4 * each relax delay
	n90 = 20*10**-6	# 90 degree pulse width for spin S
	h90 = 10*10**-6	# 90 degree pulse width for spin I
	pmis = 1.0			# mis-calibration of the pulse width for spin S
	...
	lctm = 25			# How many Vcpmg points, required for bloch[3]
...
def rcpulse_inphase_CW(de, lct, p1, c1, ofs, dw):
	...
	hamrde2 = expm(hamr.ham * hamr.dmis * 0.025/1000)
...
def rex_vs_vcpmg(hamr=Ham(), grp=1, flw=1, noise=0):
	...
	for lct in range(hamr.lctm+1):
		...
		if lct == 0:
			...
			if a <= 0:
				a = 1.0e-20
				print("Warning: logarithm becomes infinity in rex_vs_vcpmg, the point has a wrong value")
	...
	if grp == 1:
		...
		ax.scatter(magx0[:, 0], magx0[:, 1], s=13, color="blue", label="measured")
		...
		ax.set_xlabel("CPMG frequency (/sec)", fontsize=12)
		ax.set_ylabel("Rex (/sec)", fontsize=12)
...
def two_cpmg(n1, n2, n3, n4, p1, p2, p3, tcp, ldlt, lpa, lkex, lrx, ldw, jdlt, gdlt):
	...
	a = pint1[0]/pint0[0]
	if a <= 0:
		print("Warning: logarithm becomes infinity in two-cpmg")
		return 1000
	else:
...
def cpmg_plot(n, fnam, maxnc, pa, kex, rx, dw, tcp, sn, p1, p2, p3, jdlt, gdlt, drex, drey, grp=1, flw=0):
	...
	if grp == 1:
		...
		if n == 2:
			ax.scatter(drex, drey, s=8, color="blue", label="measured")
...
def start_cpmg_sim_vcpmg():
	...
	magx = rex_vs_vcpmg(grp=1, flw=0, noise=0)	# output to a graph (grp) and/or a file (flw)
	# print(magx)
...
def start_cpmg_fit(finnam='1x.inp', foutnam='1xout.txt'):
	...
	elif Ham.hpi == 7:	# simple 2*cpmg(2SyIz) - Pelement - 2*cpmg(Sx), the conventional method
		p1 = 0			# nothing: 0
...
def start_cpmg_sim_fit_2B0(b01, b02, grp=1, noise=0):
	...
	elif hamr1.hpi == 7:	# simple 2*cpmg(2SyIz) - Pelement - 2*cpmg(Sx), the conventional method
		p1 = 0				# nothing: 0
		p2 = 1				# 1/(2J), P-element: 1
		p3 = 0				# nothing: 0
		gdlt = 1.22/1000	# + hamr.n90*(60+300+60)/90/4		# each delay sandwitching the central 13C 180deg pulse (sec), 2.2 for 60-300-60 pulses
...
def start_cpmg_sim_param_plot_2d():
	...
	ax2.set_ylabel("k$_{ex}$ (/sec)", fontsize=12, fontstyle='italic')
	ax2.set_ylim([0, 1500])
	...
	ax3.set_ylabel("R$_{0}$ (/sec)", fontsize=12, fontstyle='italic')
	...
	ax5.set_ylabel("rmsd (/sec)", fontsize=12, fontstyle='italic')
...
def start_cpmg_sim_param_plot_3d():
	...
	# fontsize=10
	ax.set_xlabel("$ν_{cpmg}$ (/sec)", fontstyle='italic')
	ax.set_zlabel("R$_{2}^{eff}$ (/sec)", fontstyle='italic')
...
def start_cpmg_sim_param_plot_3d_2B0():
	...
	ax1.set_xlabel("$ν_{cpmg}$ (/sec)", fontsize=10, fontstyle='italic')
	ax1.set_zlabel("R$_{2}^{eff}$ (/sec)", fontsize=10, fontstyle='italic')
	...
	ax2.set_xlabel("$ν_{cpmg}$ (/sec)", fontsize=10, fontstyle='italic')
	ax2.set_zlabel("R$_{2}^{eff}$ (/sec)", fontsize=10, fontstyle='italic')
...
def start_cpmg_plot_2d(finnam='mag.dat', foutnam="test.pdf"):
	...
	ax = fig.add_subplot(1, 1, 1)
	# plt.rcParams["font.family"] = "Times New Roman"
	ax.scatter(drex, drey, s=13, color="blue", label="measured")
...
#start_cpmg_sim_offset()								# simulation of peak intensity ration against offset

#start_cpmg_sim_fit()									# simulation and fitting on one B0 static magnetic field
#start_cpmg_sim_fit_2B0(500, 800)						# simulation and fitting on two B0 static magnetic fields, 500: main, 800: 2nd

#start_cpmg_sim_param_cal_1B0()							# simulation of a series of RD curves against offset on one B0 static magnetic field
#start_cpmg_sim_param_plot_2d()							# 2D plot of pa, Kex, R0, dW
#start_cpmg_sim_param_plot_3d()							# 3D plot of RD curves against offset on one B0 static magnetic field
