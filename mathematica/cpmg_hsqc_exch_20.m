...
(* bloch[2] simulation of 60-300-60 composite pulse *)
...
Module[
	...
	kba = 240;	(* exchange rate from B to A *)

	jis = 140.0;	(* J(IS) coupling constant *)

	dwib = 0;	(* I's chemical shift difference of state B from that of state A *)
	dwsb = 85;	(* S's chemical shift difference of state B from that of state A *)
	...
	bixsy = 0;
	biysy = 0;
	bizsy = -kab/(kab + kba);	(* the initial state *)
	bixsz = 0;
	biysz = 0;
	bizsz = 0;
	...
	ofsrng = 5000.0;	(* The offset range of +- ofsrng, required for bloch[1] *)
	gmax = 5;			(* How many offset points are within +- ofsrng, required for bloch[1] *)
	ofs = offs;			(* The S-spin offset, required for bloch[3] *)
	lctm = 20;			(* How many Vcpmg points, required for bloch[3] *)
...
Module[
	...
	vec = MatrixExp[dmis*ham*(1.0/4/Abs[jis]-n90*2.0/Pi*2.7), vec];
...
Module[
	...
	[
		...
		(* the central 180deg S pulse *)
			5, vec = Block[{v1s = 1/4/n90*pmis, phs = tphs2}, MatrixExp[ham*n90*2, vec]], (* simple 180 deg *)
