...
#U $s = $a->sigmoid;
# make a matrix, each element of which represents sigmoid.
sub sigmoid
{
	my $self = shift;
	my @res = ();
	my $resrow;
	my ($nrow, $ncol, $row, $col, $re);
...
#U $a->mcopy($b);
# different from copy_matrix, mcopy copies matrix b to the addresses of matrix a
sub mcopy
{
	my $self = shift;
	my $other = shift;
	...
	if (($nrow != scalar(@{$other})) || ($ncol != scalar(@{$other->[0]})))
	{
		croak "The matrix sizes are inconsistent [mcopy]";
	}
	...
	for ($row = 0; $row < $nrow; $row++)
	{
		for ($col = 0; $col < $ncol; $col++)
		{
			$self->[$row][$col][0] = $other->[$row][$col][0];
			$self->[$row][$col][1] = $other->[$row][$col][1];
		}
	}
}
...
sub softmax
{
	...
	my @tmp = ();
	my $resrow;
	my ($nrow, $ncol, $row, $col, $re, $mv, $sexp, $i);
